...
OUT = r"C:\Users\KAHO\Pictures\Starwing\tools\ida_g21\out\g76_camera_possess.json"
...
# Target strings observed in the run log (camera / possess / movement / battle state)
TARGETS = [
    ("cockpit_loginparam", "Cockpit LoginParam{BattleCount: %d}"),
    ("setinputtype", "ACPP_PlayerControllerAcr::SetInputType"),
    ("initmovepram", "ACPP_CharacterEvent::InitMovePram"),
    ("sequation", "ACPP_CharacterEvent::SeqAction"),
    ("setwinlose", "SetWinLoseDemo"),
    ("moveforward", "MoveForward"),
    ("moveright", "MoveRight"),
    ("turnrate", "TurnRate"),
    ("lookup", "LookUpRate"),
    ("ebattleplayer", "E_BattlePlayer"),
    ("erespawnreturn", "E_RespawnReturn"),
    ("erespawnwait", "E_RespawnWait"),
    ("erespawncharcreate", "E_RespawnCharCreate"),
    ("setviewtarget", "SetViewTarget"),
    ("possess", "Possess"),
    ("cockpitcomponent", "CockpitComponent"),
    ("cameramode", "SetCameraMode"),
    ("camerakind", "MatchingCameraKind"),
]
...
try:
    ...
    # Named-function hints for possession / camera / character / battle
    hints = []
    for ea, name in idautils.Names():
        low = name.lower() if name else ""
        if any(k in low for k in [
            "possess", "cameramanager", "setviewtarget", "cockpit", "camera",
            "battleplayer", "respawnsequence", "playercontrollerbattle",
            "characterinit", "playermove", "inputmove", "battleplayercontroller",
        ]):
            hints.append({"ea": hex(ea), "name": name})
    sec["named_function_hints"] = hints
    results["sections"]["hint_function_count"] = len(hints)

    # Decompile the most relevant calling functions: cockpit loginparam + SetInputType + InitMovePram
    decomp = []
    seen = set()
    for label in ("cockpit_loginparam", "setinputtype", "initmovepram", "setwinlose"):
        entry = sec.get(label, {})
    ...
    results["sections"]["function_count"] = len(decomp)
...
print("FUNCTION_COUNT: %d" % results["sections"].get("function_count", 0))
