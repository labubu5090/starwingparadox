...
OUT = open(r"C:\Users\KAHO\AppData\Local\Temp\opencode\g74e_out.txt", "w", encoding="utf-8")
...
try:
    ...
    wide = [b"\xff\xfe" + n.decode().encode("utf-16-le") for n in needles]
    segs = list(idautils.Segments())
    for ndl, w in zip(needles, wide):
        total = []
        for s in segs:
            seg = ida_segment.getseg(s)
            if seg.end_ea - seg.start_ea > 0x5000:
                total += scan_seg2(seg, w)
        log("## WIDE %s : %d hits" % (ndl.decode(), len(total)))
        for ea in total[:120]:
